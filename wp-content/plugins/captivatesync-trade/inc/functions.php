...
if ( ! function_exists( 'cfm_get_snippets' ) ) :
	/**
	 * Get the snippets from Captivate
	 *
	 * @since 2.0.2
	 * @param $show_id
	 * @param $return string 'snippets' or 'count'
	 * @return array snippets or int count
	 */
	function cfm_get_snippets( $show_id, $return ) {

		$get_snippets = wp_remote_get(
		...
		if ( cfm_is_debugging_on() ) {
			$log_date = date( 'Y-m-d H:i:s', time() );
			$txt = '**GET SNIPPETS FUNCTION - ' . $log_date . '** ' . PHP_EOL . print_r( $get_snippets, true ) . '**END GET SNIPPETS FUNCTION**';
			$myfile = file_put_contents( CFMH . '/logs.txt', PHP_EOL . $txt . PHP_EOL , FILE_APPEND | LOCK_EX );
		}

		$snippets = ! is_wp_error( $get_snippets ) ? json_decode( $get_snippets['body'] ) : array();

		return ( $return == 'count' ) ? count( $snippets ) : $snippets;
	}
endif;
	...
	function cfm_update_transcript( $transcript, $episode_id ) {
		...
		if ( is_array( $transcript ) && ! empty( $transcript ) ) {
			...
			if ( function_exists( 'finfo' ) ) {
				...
				$file_info = getimagesize( $transcript );
				$mime_type = $file_info['mime'];
			}
