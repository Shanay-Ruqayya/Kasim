...
if ( ! class_exists( 'CFMH_Hosting_Sync_Front' ) ) :
	...
	class CFMH_Hosting_Sync_Front {
			...
			if ( ! class_exists( 'PWFT' ) ) {
				...
				if ( 'captivate_podcast' == $post_type ) {
					...
					if ( $cfm_episode_media_id ) {
						$output .= '<div class="cfm-player-iframe" style="width: 100%; height: 200px; margin-bottom: 20px; border-radius: 6px; overflow:hidden;"><iframe style="width: 100%; height: 200px;" frameborder="no" scrolling="no" seamless allow="autoplay" src="https://player.captivate.fm/episode/' . $cfm_episode_id . '"></iframe></div>';
					} else {
			...
			if ( is_singular( 'captivate_podcast' ) ) {
                ...
                if ( is_array( $transcript ) && ! empty( $transcript ) ) {
                	...
                	if ( ( null != $transcript['transcription_text'] && '' != $transcript['transcription_text'] ) || ( null != $transcript['transcription_html'] && '' != $transcript['transcription_html'] ) ) {
	                    ...
	                    if ( null != $transcript['transcription_text'] && '' != $transcript['transcription_text'] ) {
		 ...
		 * @return string
		 */
		public static function add_meta_card() {
			...
			if ( is_singular( 'captivate_podcast' ) ) {
				...
				$twitter_image = ( has_post_thumbnail( $post_id ) ) ? get_the_post_thumbnail_url( $post_id,  'full' ) : $cfm_episode_artwork;

				echo '	<meta property="twitter:card" content="player" />' . "\n";
				echo '	<meta property="twitter:player" content="https://player.captivate.fm/' . esc_attr( $cfm_episode_id ) . '/twitter/">' . "\n";
				...
				echo '	<meta property="twitter:image" content="' . esc_attr( $twitter_image ) . '" />' . "\n";
